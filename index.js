...
app.get(
  ...
  (req, res) => {
    if (req.user) {
      // If user is authenticated, redirect to /app regardless of API keys
...
const sendErrorResponse = async (res) => {
  try {
    return res.send({
      messages: [
        {
          text: "I apologize, but I need my API keys to help you properly. Please make sure both ElevenLabs and Gemini API keys are configured!",
...
const checkRequiredEnv = async (req, res, next) => {
  if (!ELEVENLABS_API_KEY || GEMINI_AI_API_KEY === "") {
...
// Modified chat endpoint to check API keys only when actually using the bot
app.post("/chat", async (req, res) => {
  try {
    const userMessage = req.body.message;
    
    // First message (no user input) - show welcome message regardless of API keys
    if (!userMessage) {
      res.send({
        messages: [
          {
    ...
    // Check API keys only when processing actual user message
